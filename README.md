...
4. Then, you could ssh to the node as follows:
   ...
   ```
   ps -ef | egrep "linux-system-roles.logging.tests" | egrep -v grep | awk '{print $28}' | awk -F':' '{print $3}' | awk -F'-' '{print $1}'
   ```
5. When the debugging is done, run `ps -er | grep standard-inventory-qcow2` and kill the pid to clean up the node.
